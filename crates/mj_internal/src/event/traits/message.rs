use crate::contact::{Bot, ContactTrait, Friend};
use crate::event::{
    BotActiveEventTrait, BotEventTrait, BotPassiveEventTrait, CancellableEventTrait,
    ...
    Self: MiraiEventTrait + BotPassiveEventTrait + OtherClientEventTrait,
{
    ...
    fn get_sender(&self) -> Sender {
        let jvm = Jvm::attach_thread().unwrap();
        let instance = jvm.invoke(&self.get_instance(), "getSender", &[]).unwrap();
    ...
    fn get_subject(&self) -> Subject {
        let jvm = Jvm::attach_thread().unwrap();
        let instance = jvm.invoke(&self.get_instance(), "getSubject", &[]).unwrap();
