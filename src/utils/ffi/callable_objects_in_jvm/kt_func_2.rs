...
pub struct KtFunc2<'a, P1, P2, R, F>
    where
        P1: FromInstance,
        P2: FromInstance,
        F: Fn(P1, P2) -> R,
...
impl<'a, P1, P2, R, F> GetEnvTrait for KtFunc2<'a, P1, P2, R, F>
    where
        P1: FromInstance,
        P2: FromInstance,
        F: Fn(P1, P2) -> R,
...
impl<'a, P1, P2, R, F> KtFunc2<'a, P1, P2, R, F>
    where
        P1: FromInstance,
        P2: FromInstance,
        F: Fn(P1, P2) -> R,
        R: GetEnvTrait + FromInstance,
{
    ...
    pub fn new(closure: &'a F) -> Pin<Box<KtFunc2<'a, P1, P2, R, F>>> {
...
impl<'a, P1, P2, R, F> Drop for KtFunc2<'a, P1, P2, R, F>
    where
        P1: FromInstance,
        P2: FromInstance,
        F: Fn(P1, P2) -> R,
...
{
    fn drop(&mut self) {
        let kt_func_2: *mut dyn Fn(DataWrapper<Instance>) -> Instance =
            unsafe { transmute(self.internal_closure_raw) };
        let boxed = unsafe { Box::from_raw(kt_func_2) };
        drop(boxed)
    }
}
